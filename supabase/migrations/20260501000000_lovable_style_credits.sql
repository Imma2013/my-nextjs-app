...
create index if not exists usage_events_free_daily_idx
...
declare
  ...
  v_free_daily_used numeric(10,2);
  ...
  v_tomorrow_start timestamptz := ((date_trunc('day', now() at time zone 'utc') + interval '1 day') at time zone 'utc');
...
begin
  ...
  if found and v_usage.status = 'completed' then
    return jsonb_build_object(
      ...
      'billingSource', coalesce(v_usage.billing_source, case when v_usage.free_tailor_used then 'free_daily' else 'credits' end),
  ...
  elsif v_topup_available >= p_cost then
    v_billing_source := 'topup_usage';
  else
    v_billing_source := 'paid_usage';
  end if;
