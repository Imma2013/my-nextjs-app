...
export default function ResumeDocumentLayout({ resume, onEdit }: { resume: ResumeLike; onEdit?: (edit: EditPayload) => void }) {
  ...
  return <div className="resume-document mx-auto min-h-[900px] max-w-[850px] bg-white px-16 py-14 text-slate-800 shadow-sm ring-1 ring-slate-200">
    ...
    <section className="mb-5"><h3 className="border-b border-slate-800 font-serif text-lg font-semibold uppercase text-slate-700">Skills</h3><div className="mt-2 text-sm leading-6 text-slate-600">{editable ? <div className="flex flex-wrap gap-2">{(skills.length ? skills : ['Skills will appear here after upload.']).map((s: any, i: number) => <EditableText key={i} value={typeof s === 'string' ? s : label(s, '')} className="rounded border border-slate-200 px-2 py-1" onSave={value => edit({ operation: 'replace', path: `skills.${i}`, value })} />)}{resume && <button type="button" onClick={() => edit({ operation: 'add', path: 'skills', value: 'New Skill' })} className="rounded border border-dashed border-blue-300 px-2 py-1 text-xs text-blue-600">+ skill</button>}</div> : (skills.length ? skills : ['Skills will appear here after upload.']).map((s: any) => typeof s === 'string' ? s : label(s, '')).filter(Boolean).join(', ')}</div></section>
